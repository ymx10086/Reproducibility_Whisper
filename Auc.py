...
def f_action(label, loss):
    ...
    print(f'F1 : {f1:7.6f}, F2 :  {f2:7.6f}, PR_AUC : {pr_auc:7.6f}')
...
def analyze_action(tag: str, malicious_addr: List[str]) -> None:
    ...
    for addr in malicious_addr:
        ...
        int_malicious_addr.append(addr)

    print("int_malicious_addr: ", int_malicious_addr)
    ...
    for file in traget_files:
        with open(save_path + tag + '/' + file, 'r') as f:
            ...
            for entery in ls:
                if entery['address'] in int_malicious_addr:
                    abnormal.extend([*(entery['distence'] for _ in range(entery['packet_num']))])
                else:
                    normal.extend([*(entery['distence'] for _ in range(entery['packet_num']))])
    ...
    print(f'ROC_AUC : {roc_auc:7.6f}, ERR : {err:7.6f}, ', end='')
...
if __name__ == '__main__':
    ...
    analyze_action(tag='SYNDOS', malicious_addr=["192.168.3.11"])
